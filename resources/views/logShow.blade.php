        ...
        <fieldset disabled>
            <div class="container text-left">
                ...
                <p id="date_receipt">Дата поступления:
                    <strong style="color: dodgerblue">{{ $log->logReceipt->date_receipt }}</strong></p>
                <p id="string_time_receipt">Время поступления:
                    <strong style="color: dodgerblue">{{ $log->logReceipt->time_receipt }}</strong></p>
                <hr>
                ...
                <p id="number_phone_representative">Номер телефона законного представителя, лица, которому может
                    ...
                    <strong style="color: dodgerblue">{{ $log->logReceipt->phone_agent }}</strong></p>
                <p id="delivered">Пациент доставлен (направлен):
                    <strong style="color: dodgerblue">{{ $log->logReceipt->delivered }}</strong></p>
                <p id="fact_alcohol">Факт употребления алкоголя и иных психоактивных веществ,
                    ...
                    <strong style="color: dodgerblue">{{ $log->logReceipt->fact_alcohol }}</strong></p>
                <p id="date_time_alcohol">Дата и время взятия пробы:
                    <strong style="color: dodgerblue">{{ $log->logReceipt->datetime_alcohol }}</strong></p>
                <p id="result_research">Результаты лабораторных исследований:
                    <strong style="color: dodgerblue">{{ $log->logReceipt->result_research }}</strong></p>
                <p id="department_medical_organization">Отделение медицинской организации, в которое
                    направлен пациент:
                    <strong style="color: dodgerblue">{{ $log->logReceipt->section_medical }}</strong></p>
                <hr>
                ...
                <p id="date_time_inform">Дата и время сообщения законному представителю, иному лицу
                    ...
                    ее исходе:
                    <strong style="color: dodgerblue">{{ $log->logDischarge->datetime_inform }}</strong></p>
                <hr>
                <h4> В случае отказа в госпитализации в стационар, дневной стационар</h4>
                <p id="reason_refusal">Причина отказа в госпитализации:
                    <strong style="color: dodgerblue">{{ $log->logReject->reason_refusal }}</strong></p>
                <p id="full_name_medical_worker">Фамилия, имя, отчество (при наличии) медицинского
                    ...
                    <strong style="color: dodgerblue">{{ $log->logReject->name_medical_worker }}</strong></p>
                <p id="additional_information">Дополнительные сведения:
                    <strong style="color: dodgerblue">{{ $log->logReject->add_info }}</strong></p>
            </div>
        </fieldset>
    </div>
